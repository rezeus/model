...
class Model {
  ...
  static get setterNames() {
    const ownProperties = Object.getOwnPropertyDescriptors(this.prototype);
    let ownProperty;
    const setterNames = Object.keys(ownProperties).filter((propName) => {
      ownProperty = ownProperties[propName];
  ...
  async save() {
    ...
    const wasDirty = this[PRIV].isDirty;
  ...
  toJSON(useExcludedFieldNames = true) {
    ...
    // DOC Underscore prefixed field names are not going to be returned in the JSON
      ...
      .reduce((obj, fieldName) => {
        currRawField = rawFields[fieldName];

        if (typeof currRawField === 'object' && Array.isArray(currRawField)) {
          // eslint-disable-next-line no-param-reassign
          obj[fieldName] = currRawField.map((currRawFieldItem) => {
            if (currRawField instanceof Model) {
              return currRawField.toJSON(useExcludedFieldNames);
            }

            return currRawFieldItem;
          });
        } else {
          // eslint-disable-next-line no-param-reassign
          obj[fieldName] = currRawField;
        }
