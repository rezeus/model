...
class Model {
  ...
  toJSON() {
    if (this.constructor.jsonExcludedFieldNames.length > 0) {
      return Object.freeze(omit({ ...this[PRIV].fields }, this.constructor.jsonExcludedFieldNames));
    }

    return Object.freeze({ ...this[PRIV].fields });
  }
  ...
  static create(fields) {
    return new Promise((resolve, reject) => {
      ...
      // TODO 'beforeCreate' Hook (invoke with `inst`)

      inst.save()
        .then(() => {
          resolve(inst);

          // TODO 'afterCreate' Hook (invoke with `inst`)
        })
